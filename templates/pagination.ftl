...
<#--
http://freestyle-developments.co.uk/blog/?p=343
 * Pagination macros.
 *
...
<a href="?field=${data.sortField}&amp;page=${pageNumber}&amp;size=${data.pageSize}&amp;direction=${data.sortDirection}"${attributes}>${text?html}</a>
</#macro>
...
<#--<@spring.messageArgsText "pagination.counter", [data.pageNumber + 1, pagesAvailable], "{0} of {1}" />-->
    ${data.pageNumber + 1} of ${pagesAvailable}
</#macro>
...
<a href="?field=${field}&amp;page=${data.pageNumber}&amp;size=${data.pageSize}&amp;direction=${direction}"${attributes}>${text?html}</a>
</#macro>